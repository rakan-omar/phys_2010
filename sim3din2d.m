...
% from the kinematic equations
    ...
    if z_position(i) < 0 %to counter error due to rounding
        z_position(i) = 0;
        x_position(i) = x_speed*2*time_at_peak;
        y_position(i) = y_speed*2*time_at_peak; %0
    end
end
%% plotting 
...
plot(x_position(1, 1:explosion_step), z_position(1, 1:explosion_step), ...
    'r-', 'LineWidth', 2);
grid on
hold on
% plot3(x_position, y_position, z_position,'g--','Linewidth',1.2)
plot(x_position, z_position,'g--','Linewidth',1.2)
title('3 dimensional explosion projection')
xlabel('x position (m)')
ylabel('height (m)')
%zlabel('height (m)')
grid on
...
%     plot3(fragment_x_positions(i, :), fragment_y_positions(i, :), fragment_z_positions(i, :),'LineWidth',0.8);
    plot(fragment_x_positions(i, :), fragment_z_positions(i, :),'LineWidth',0.8);
    grid on
    hold on
end
...
plot(centre_of_mass(1, :),centre_of_mass(3, :), 'b-', 'lineWidth', 2)
