...
% In this program it is assumed that the bomb starts in one piece and then
...
order = zeros(2, n_fragments); %the nth element (of the first row) 
% is the index of the nth ball to reach the floor. The second keeps track
% of which time step this occurs at
while (midair)
    t = tau*k;
    k = k+1;
    for i=1:n_fragments
...
%                 t_to_ground_2 = (y_speed_fragments(i)/g) - (((y_speed_fragments(i)^2) + (2*g*explosion_position(2)))^0.5)/g;
                ...
                order(1:2, reached_ground) = [i, k-1];
...
mass_sum = zeros(1, n_fragments); %total mass still in air
mass_sum(1) = M;

for i=1:n_fragments-1
    mass_sum(i+1) = mass_sum(i) - fragment_masses(order(1, i));
end

centre_of_mass = zeros(2, k);
next = 1; %order index of next fragment to drop
for j=1:k
    for i=next:n_fragments
%         fprintf('%.1f \n',i);
        index = order(1, i);
        centre_of_mass(1, j) = (centre_of_mass(1, j) + (fragment_x_positions(index, j) * fragment_masses(index)));
        centre_of_mass(2, j) = (centre_of_mass(2, j) + (fragment_y_positions(index, j) * fragment_masses(index)));
%         fprintf("ball %.1f \n", index);
    end
    if order(2, next) == j
        if next == n_fragments-1
            break
        end
        next = next+1;
    end
...
previous_index = 1;
for i=1:n_fragments
    fragment_index = order(1, i);
    frame_index = order(2, i);
    centre_of_mass(:, previous_index:frame_index) = centre_of_mass(:, previous_index:frame_index)/mass_sum(i);
    previous_index = frame_index;
end

index = order(2, n_fragments-1);
plot(centre_of_mass(1, 1:index-1), centre_of_mass(2, 1:index-1),'b-', 'lineWidth', 2) % plotting the centre of mass trajectory    
