...
midair = true;
frames_to_ground = zeros(1, n_fragments);
...
while (midair)
    ...
    for i=1:n_fragments
        ...
        if (fragment_y_positions(i, k) < 0) && all_midair(i) 
            all_midair(i) = 0;
            frames_to_ground(i) = k-1;
...
for i=1:n_fragments
    plot(fragment_x_positions(i, 1:frames_to_ground(i)), ...
        fragment_y_positions(i, 1:frames_to_ground(i)),'LineWidth',0.8);
    grid on
    hold on
end
...
for j=1:k
    ...
    if centre_of_mass(2, j) < 0
        frames_to_ground(n_fragments+1) = j-1;
        break
    end
end
centre_of_mass = centre_of_mass/M;

plot(centre_of_mass(1, 1:frames_to_ground(n_fragments+1)), ...
    centre_of_mass(2, 1:frames_to_ground(n_fragments+1)),'b-', 'lineWidth', 2) % plotting the centre of mass trajectory
